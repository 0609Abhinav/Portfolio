...
/* ─────────────────────────────────────────────────────────────
   AIAssistant — Floating AI Chat Widget (Mobile-First)
   • Desktop: slides up from bottom-right corner (420px panel)
   • Mobile:  full-screen bottom sheet (swipe-friendly)
   • Text + voice (STT) input
   • AI voice (TTS) responses
   • Quick-action chips
   • Markdown-lite rendering
   • iOS Safari graceful degradation for STT
   • Body scroll lock on mobile
   • 16px font on input (prevents iOS auto-zoom)
───────────────────────────────────────────────────────────── */
...
export default function AIAssistant() {
  ...
  // ── Populate input with STT transcript ──
  useEffect(() => {
    if (transcript) setInput(transcript);
  }, [transcript]);

  // ── Focus input when chat opens (desktop only — causes keyboard flash on mobile) ──
  useEffect(() => {
    if (open && !isMobile) {
  ...
  // ── Panel style changes based on device ──
  const panelStyle = isMobile
    ? {
        ...
        boxShadow: "0 -8px 40px rgba(0,255,255,0.1), 0 -4px 80px rgba(139,92,246,0.08)",
        zIndex: 9998,
        display: "flex",
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            {/* ── Mobile drag handle ── */}
            {isMobile && (
              <div style={{ display:"flex", justifyContent:"center", paddingTop:10, paddingBottom:4, flexShrink:0, position:"relative", zIndex:1 }}>
            ...
            {/* ── Header ── */}
            <div style={{
              padding: isMobile ? "10px 16px 10px" : "16px 20px 14px",
            ...
            >
              {QUICK_CHIPS.slice(0, isMobile ? 4 : 5).map((chip) => (
                <motion.button
                  key={chip}
                  whileTap={{ scale:0.95 }}
            ...
            <div
              style={{
                ...
                padding: isMobile ? "8px 14px 12px" : "8px 16px 12px",
            ...
            <div style={{
              ...
              /* Stick above iOS home indicator */
              ...
              {supported.stt && (
                ...
                >
                  <MicIcon isActive={isListening} size={isMobile ? 18 : 16} />
                </motion.button>
              )}

              {/* STT not supported hint (iOS Safari) */}
              {!supported.stt && isMobile && (
                <div style={{
                  flexShrink:0,
                  width:44, height:44,
                  display:"flex", alignItems:"center", justifyContent:"center",
                  ...
                  <MicIcon isActive={false} size={18} />
                </div>
              )}
              ...
              >
                <SendIcon size={isMobile ? 18 : 16} />
              </motion.button>
            </div>

            {/* ── STT unsupported notice (iOS Safari) ── */}
            {!supported.stt && (
              <div style={{
                padding:"0 14px 8px",
                fontSize:"0.62rem",
                ...
                🎤 Voice input needs Chrome browser · Text chat fully works
              </div>
            )}
            ...
            <div style={{
              textAlign:"center",
              padding: !supported.stt ? "2px 14px 10px" : "6px 14px 10px",
              fontSize:"0.62rem",
              color:"#1e293b",
              letterSpacing:"0.1em",
...
function MicIcon({ isActive, size = 16 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none"
